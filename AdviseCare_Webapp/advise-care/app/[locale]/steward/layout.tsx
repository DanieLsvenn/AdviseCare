...
export default async function StewardLayout({ children }: LayoutProps<"/[locale]/steward">) {
  const t = await getTranslations("nav");
  const session = await apiGet<{ steward: Account; term: AcademicTerm }>("/api/session");

  const navItems: NavItem[] = [
    { href: "/steward/purposes", label: t("steward.purposes"), icon: "checklist" },
    ...
    { href: "/steward/erasure", label: t("steward.erasure"), icon: "delete_sweep" },
    {
  ...
  return (
    <AppShell
      ...
      title={<ActiveNavTitle items={navItems} fallback={t("steward.purposes")} />}
